...
def preprocess_ffhq(img_path,lm_path,save_path,target_size=256,cate=None):
    ...
    print(os.path.join(img_path.replace(img_name,''),'poses',img_name.replace('png','mat')))
    print(os.path.join(save_path,'poses',img_name.replace('png','mat')))
...
def preprocess_data(raw_dataset_path, cate='ffhq'):
    ...
    if cate == 'ffhq':
        ...
        all_lm_path = [os.path.join(raw_dataset_path,'detections',f.split('/')[-1].replace('png','txt')) for f in all_img_path]
